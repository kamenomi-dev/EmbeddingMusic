...
Logger::Logger(std::ostream& s)
    : _file(),
      _log(s),
      _level(logger_level::LL_INFO),
      _line_level(logger_level::LL_LOG),
      _default_line_level(logger_level::LL_LOG)
...
void Logger::flush() {
  if (_line_level >= _level) {
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, LOG_TAG, "%s",
                        _log_ss.str().c_str());  // #todo level convert
    _log_ss.str("");
#endif
    if (_file.is_open())
      _log.flush();
...
Logger& Logger::operator<<(const logger_level::_logger_level level) {
  ...
  while (_spinlock.test_and_set(std::memory_order_relaxed))
...
std::string Logger::get_time() const {
  ...
  time(&rawtime);
  strftime(time_buf, sizeof time_buf, "%FT%TZ", gmtime(&rawtime));
...
inline const char* Logger::level_str(const logger_level::_logger_level level) {
  switch (level) {
    case logger_level::LL_LOG:
      return (" LOG  ");
    case logger_level::LL_DEBUG:
      return ("DEBUG ");
    case logger_level::LL_INFO:
      return (" INFO ");
    case logger_level::LL_NOTICE:
      return ("NOTICE");
    case logger_level::LL_WARN:
      return (" WARN ");
    case logger_level::LL_ERROR:
      return ("ERROR ");
    case logger_level::LL_FATAL:
      return ("FATAL ");
    default:
      assert(false);
      return nullptr;
